...
% Make the directories for the statistical results and figures
%
[~,~] = mkdir('Results');
[~,~] = mkdir('Figures');
...
for run = 1:Rmax 
    ...
    if total_scans ~= 0
        ...
        GLOBAL_META = zeros(NSUBS(run),1);
        GLOBAL_VAR_MEAN = zeros(NSUBS(run),1);
        GLOBAL_SR = zeros(NSUBS(run),1);
        GLOBAL_IGNITE = zeros(NSUBS(run),1);
        GLOBAL_KOP = zeros(NSUBS(run),Tmax);
...
%%
% 
% Now export the signatures to analyse in R using 01_Global_Metrics-Rmd in
% the folder Statistics. 
% Results will be in the Results folder
% Figure will be in the Figures folder
%
% 

data_labels = { 'SUB','COND','MKOP','MVAR','SSPR','MINT'};
DESC = {'Metastability','Dynamical_Complexity','Dynamical_Agility','Ignition_Flexibility'};

conditions = param.cond;

%
% Assemble table elements for each condition and append to table
%

for run = 1:Rmax

    GS = load([DRV_DATA num2str(run) '/ALL_Sigs']);
    GM = GS.GLOBAL_META;
    GV = GS.GLOBAL_VAR_MEAN;
    SSR = GS.GLOBAL_SR;
    GT = GS.GLOBAL_IGNITE;

    
    COND = repmat(cond(run,:),NSUBS(run),1);
    SUB = (1:NSUBS(run));
    
    rt=table('Size', [NSUBS(run), numel(data_labels)],'VariableTypes', {'double','categorical','double','double','double','double'},'VariableNames',data_labels);
    ...
    writetable(rt,[DRV_DATA num2str(run) '/Regtable_run' num2str(run) '.csv'])
    writecell(DESC, [DRV_DATA num2str(run) '/GLOBAL_Descriptors' num2str(run) '.csv'])
end
